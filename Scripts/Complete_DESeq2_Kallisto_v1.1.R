...
#Import Kallisto Count Matrix; FishTEDB TEs are annotated with a #, thus the comment character must be blank when importing
Count_Matrix <- read.table("kallisto_count_matrix", header = T, comment.char = "")
...
my.heatmap.title <- paste("Aging significant (FDR<5%) PE Kallisto, ", length(TE_chars), " TEs",sep="")
