	...
	then
	    for f in $(find "." -name '*_1_hardtrim_val_1.fq.gz')
			do
    			...
    			STAR --genomeDir $Indexed_Genome_Dir --readFilesIn $f $inf2 --readFilesCommand gzcat --runThreadN 2 --outFilterMultimapNmax 100 --outFilterIntronMotifs RemoveNoncanonicalUnannotated --alignEndsProtrude 10 ConcordantPair --limitGenomeGenerateRAM 60000000000 --outSAMtype BAM SortedByCoordinate --outFileNamePrefix $oFname
			done
		...
		for f in $(find "." -name '*_1_hardtrim_trimmed.fq.gz')
			do
		    	...
		    	oFname="${Genomic_output_directory}/${of}"
		    	STAR --genomeDir $Indexed_Genome_Dir --readFilesIn $f --readFilesCommand gzcat --runThreadN 2 --outFilterMultimapNmax 100 --outFilterIntronMotifs RemoveNoncanonicalUnannotated --alignEndsProtrude 10 ConcordantPair --limitGenomeGenerateRAM 60000000000 --outSAMtype BAM SortedByCoordinate --outFileNamePrefix $oFname
			done
...
bam_names=$(ls -p | grep "Aligned.sortedByCoord.out.bam" | tr '\n' ' ')
	...
	then
		if [ $Fraction = "Y" ]
		then
		$FtCts_Path -O -p -M --fraction --primary -a $genomic_gtf -o $FtCts_output $bam_names
		else
		$FtCts_Path -O -p --fraction -a $genomic_gtf -o $FtCts_output $bam_names
		fi
	else
		if [ $Fraction = "Y" ]
		then
		$FtCts_Path -O -M --fraction --primary -a $genomic_gtf -o $FtCts_output $bam_names
		else
		$FtCts_Path -O --primary -a $genomic_gtf -o $FtCts_output $bam_names
		fi
	fi
else
